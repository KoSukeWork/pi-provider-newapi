...
即使模型尚未发现，provider 也会出现在 `/login` 中。登录后 Pi 会存储 API Key、刷新 provider 目录，模型随即可以使用：
...
- 打开 `/model` 会在后台刷新已配置的 NewAPI 目录。
- `pi update --models` 会强制刷新目录。
- 成功的目录按 provider 存储在 Pi 的 `<agentDir>/models-store.json`。
- 离线模式下，Pi 会恢复最近一次成功的目录，且不会向 NewAPI 发出请求。
- 刷新失败会保留最后一次有效缓存。可选的 `/api/ratio_config` 端点失败不会阻止发现；获取新目录时 `/v1/models` 必须成功。
...
每个 provider 都有独立的扩展配置、Pi 凭据和目录缓存。
