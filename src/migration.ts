...
function moveConfigToCurrentPath(version: ConfigVersion, configPath: string): ConfigVersion {
	const legacyPath = getLegacyConfigPath();
	...
	try {
		...
		return { ...version, path: configPath };
	} catch (error) {
		// Another process may have completed the migration after our existence checks.
		if (existsSync(configPath)) return { path: configPath, schemaVersion: readSchemaVersion(configPath) };
...
function upgradeSchemaVersion(version: ConfigVersion): ConfigVersion {
	if (version.schemaVersion >= CONFIG_SCHEMA_VERSION || !existsSync(version.path)) return version;
	try {
		const parsed = JSON.parse(readFileSync(version.path, "utf-8")) as unknown;
		if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) return version;
		...
		return { ...version, schemaVersion: CONFIG_SCHEMA_VERSION };
	} catch (error) {
		warnMigrationOnce(
...
export function migrateConfig(configPath = getCurrentConfigPath()): ConfigVersion {
	const version = upgradeSchemaVersion(moveConfigToCurrentPath(getConfigVersion(configPath), configPath));
